...
export default function Grammar() {
  ...
  const handleClick = async () => {
    if (!prompt) return;
    const reqPrompt = `Correct the following sentence for grammar:\n\n${prompt}`;
    setLoading(true);
    ...
    try {
      ...
      if (result) {
        ...
        setResult("An error occurred while checking grammar. Please try again later");
      }
    } catch (error) {
  ...
  return (
    <main className="main">
      ...
      <div className="container">
        <div className="row">
          {arrayItems
            .filter((item) => item.id === "grammarCorrection")
            .map((item) => (
          ...
          <div className="col-lg-10 mx-auto">
            <textarea
              ...
              placeholder="Write a sentence to check grammar..."
